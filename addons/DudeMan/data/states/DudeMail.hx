...
function create() {
    ...
    if (FlxG.save.data.dudemailsUnlocked >= 3) {
        makeMail("3", 222, "icon3", "sender3", "subject3", "fruity", "FRUITY", "DONT IGNORE THIS MESSAGE.");
    }
    if (FlxG.save.data.dudemailsUnlocked >= 4) {
        makeMail("4", 333, "icon4", "sender4", "subject4", "puzzle", "Puzzle", "My Game.");
    }
    if (FlxG.save.data.dudemailsUnlocked >= 6) {
        makeMail("5", 444, "icon5", "sender5", "subject5", "puzzle", "Puzzle", "Please");
    }
    if (FlxG.save.data.dudemailsUnlocked >= 5) {
        makeMail("6", 555, "icon6", "sender6", "subject6", "bee", "Bee", "Hey c:");
    }
    if (FlxG.save.data.dudemailsUnlocked >= 7) {
        makeMail("7", 666, "icon7", "sender7", "subject7", "damie", "damie_an", "IT IS NOT ME");
    }
    if (FlxG.save.data.dudemailsUnlocked >= 8) {
        makeMail("8", 777, "icon8", "sender8", "subject8", "beebo", "Beebo", "Hey dude. Not cool.");
    }
...
function makeMail(number, posY, iconName, senderName, subjectName, icon, sender, subject) {
    ...
    if (number == "8") {
        mailSlot8 = new FlxSprite(100, 100).loadGraphic(Paths.image(slot));
        slotToBe = mailSlot8;
    }
...
function makeMailScreen() {
    ...
    mailInfo8 = new FlxText(50, 50, 670, "placeholder", 0);

    for (mails in [mailInfo1, mailInfo2, mailInfo3, mailInfo4, mailInfo5, mailInfo6, mailInfo7, mailInfo8]) {
    ...
    mailText8 = new FlxText(50, 50, 670, "placeholder", 0);
    
    for (mails in [mailText1, mailText2, mailText3, mailText4, mailText5, mailText6, mailText7, mailText8]) {
        mails.setFormat(Paths.font("Bahnschrift.TTF"), 20, FlxColor.WHITE, "left");
    ...
    if (FlxG.save.data.dudemailsUnlocked >= 5) {
        mailInfo5.text = "To: BeeFunk@dudemail.com\nFrom: HotSexyDemon972@dudemail.com\nSubject: Please\n\n\n\n\n\n\n\n=======================\nTo: HotSexyDemon972@dudemail.com\nFrom: BeeFunk@dudemail.com\nSubject: uhm\n\n\n\n=========================\nTo: BeeFunk@dudemail.com\nFrom: HotSexyDemon972@dudemail.com\nSubject: <NO SUBJECT>";
    ...
    if (FlxG.save.data.dudemailsUnlocked >= 8) {
        mailInfo8.text = "To: dudemanwastakenwastaken@dudemail.com\nFrom: beeboisbeeboyes5675432122333:3:3:3@dudemail.com\nSubject: Hey dude. Not cool.";
        mailText8.text = "\n\n\n\nI saw you in the drive through at Wendy's and I;m kinda PISSED you ordered 74 FUCKING BURGERS AND IT TOOK 3 HOURS TO GET TO THE DFUCKING WINDOW TO GET MY FOOD AND WHEN I GOT THERE IT WAS FUCKING COLD YOU PISS ME OFF I HOPE YOU NEVER SLEEP FOR MORE THAN 30 MINUTES FOR THE REST OF YOURE LIFE FUCK YOU\n\nP.S. Me wife gave me a divorce slips whenever i was typing this, i regret saying anything i am so sorry. please help me get my wife back.\n\nSincerely, Beebo.";
    }
}
...
function update() {
    ...
    if (FlxG.save.data.dudemailsUnlocked >= 8) {
        if (FlxG.mouse.overlaps(mailSlot8) && FlxG.mouse.justPressed) {
            selectMail(8);
        }
    }
    ...
    if (selectedMail == 8) {
        mailInfo8.alpha = 1;
        mailText8.alpha = 1;
        mailSlot8.loadGraphic(Paths.image(slotSelected));
    }
    else {
        mailInfo8.alpha = 0;
        mailText8.alpha = 0;
        mailSlot8.loadGraphic(Paths.image(slot));
    }
}

function selectMail(mailToSelect) {
    selectedMail = mailToSelect;
