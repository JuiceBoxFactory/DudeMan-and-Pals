...
{
    float edge0 = (pos - iRange);
    float edge1 = (pos + iRange);
...
void mainImage()
{
    ...
    float r = texture(iChannel0, uv + (offsetR * Amt)).r;
    float g = texture(iChannel0, uv + (offsetG * Amt)).g;
